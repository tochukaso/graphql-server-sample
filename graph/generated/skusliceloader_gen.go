...
type SkuSliceLoader struct {
	...
	cache map[int][]*model.Sku

	// the current batch. keys will continue to be collected until timeout is hit,
	// then everything will be sent to the fetch method and out to the listeners
	batch *skuSliceLoaderBatch

	// mutex to prevent races
	mu sync.Mutex
}

type skuSliceLoaderBatch struct {
	keys    []int
	data    [][]*model.Sku
	error   []error
	closing bool
...
// LoadThunk returns a function that when called will block waiting for a Sku.
...
func (l *SkuSliceLoader) LoadThunk(key int) func() ([]*model.Sku, error) {
	...
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() ([]*model.Sku, error) {
			return it, nil
		}
	}
	if l.batch == nil {
		l.batch = &skuSliceLoaderBatch{done: make(chan struct{})}
	}
	batch := l.batch
...
func (l *SkuSliceLoader) LoadAll(keys []int) ([][]*model.Sku, []error) {
	results := make([]func() ([]*model.Sku, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	skus := make([][]*model.Sku, len(keys))
	errors := make([]error, len(keys))
	for i, thunk := range results {
		skus[i], errors[i] = thunk()
	}
	return skus, errors
}

// LoadAllThunk returns a function that when called will block waiting for a Skus.
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
...
func (l *SkuSliceLoader) unsafeSet(key int, value []*model.Sku) {
	if l.cache == nil {
		l.cache = map[int][]*model.Sku{}
	}
	l.cache[key] = value
}
...
func (b *skuSliceLoaderBatch) keyIndex(l *SkuSliceLoader, key int) int {
...
func (b *skuSliceLoaderBatch) startTimer(l *SkuSliceLoader) {
	time.Sleep(l.wait)
	l.mu.Lock()
...
func (b *skuSliceLoaderBatch) end(l *SkuSliceLoader) {
